...
export const COMMANDS: CommandSpec[] = [
  ...
  {
    name: "/remove",
    detail: "remove the worker: worktree, branch, rail row",
    workerOnly: true,
    shortcut: "ctrl+r",
    aliases: ["/rm", "/r"],
  },
...
export function resolveCommand(word: string): CommandSpec | null {
  ...
  return (
    COMMANDS.find((c) => c.name === token || c.aliases?.includes(token)) ?? null
  );
}
...
export function completionsFor(
  ...
  if (token.startsWith("/") && start === 0) {
    const available = COMMANDS.filter(
      (c) => !c.workerOnly || ctx.target === "worker",
    );
    // match the long form and every alias, so "/q" finds "/quit"
    const items = rank(available, token, (c) =>
      commandForms(c).join(" "),
    ).map<Suggestion>((c) => ({
      ...
      detail: [
        c.detail,
        `(${[...(c.aliases ?? []), c.shortcut].filter(Boolean).join(", ")})`,
      ].join("  "),
      kind: "command",
    }));
    ...
    const agentItems = rank(
      ...
      label: `/${c.name}`,
      detail: [c.description, c.argumentHint, c.source ? `[${c.source}]` : ""]
        .filter(Boolean)
        .join("  "),
      kind: "agent",
    }));
    ...
    return all.length > 0
      ? { items: all.slice(0, MAX_SUGGESTIONS), start, token }
      : null;
  }

  if (token.startsWith("@")) {
    const query = token.slice(1);
    const workers = rank(ctx.workers, query, (w) => w.name).map<Suggestion>(
      (w) => ({
        value: `@${w.name}`,
        label: `@${w.name}`,
        detail: w.detail,
    ...
    const files = rank(ctx.files, query, (f) => f).map<Suggestion>((f) => ({
      value: `@${f}`,
      label: `@${f}`,
      kind: "file",
    }));
