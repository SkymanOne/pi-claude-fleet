...
- 2026-08-30 — Rust does not reap detached children on drop (Node's `unref()` did it for free), so a spawned monitor lingered as a zombie whose pid kept looking alive. `spawn_monitor` uses `tokio::process::Command` (which has the safe `process_group(0)`; `pre_exec` is `unsafe` and forbidden) and reaps via a background task. Apply the same care wherever a detached child is spawned.
...
- **Test flakes were under repair at cutover, not yet finished.** Two families, seen to rotate under full-suite parallel load: git subprocess transients (`src/git.rs`, `src/ops/spawn.rs`, `src/paths.rs` — a shared bounded-retry helper was the planned fix) and reading `run.json` immediately after the event that precedes the state flush (`tests/worker_monitor.rs` — poll for the state condition instead). One further flake in frozen `ops/mod.rs:157` fires at roughly 3% per run. The repair run (rust-fixes) was still working at cutover: best observed clean streak 9 consecutive full-suite runs, no report written yet — check its report before trusting the suite as flake-free.
